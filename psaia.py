...
import glob
import pdb
PSAIA_PATH='/scratch/groups/rondror/ppi/pairpred/lib/psaia-src/PSAIA_1.0_source/bin'
...
def runPSAIA(fname,ofname=None):
    ...
    cmdstr='echo y | psa '+fconfig+' '+flist
    #pdb.set_trace()
    ecode=os.system(cmdstr)
    os.remove(flist)
    os.remove(fconfig)
    xfname=glob.glob(os.path.join(cdir,xf[0:4]+'*bound.tbl'))[0]
    pdict=make_psaia_dict(xfname)

    if rmv: #cleanup
        for x in glob.glob(os.path.join(cdir,xf[0:4]+'*bound.tbl')):
            os.remove(x)
